...
it("changing loss rate DOES NOT disrupt jitter sequence", () => {
  const clean = run(0);
  const lossy = run(0.2);
  // Packets that arrive in the lossy run also appear in the same ORDER in the clean run.
...
it("sufficient jitter produces out-of-order packets", () => {
